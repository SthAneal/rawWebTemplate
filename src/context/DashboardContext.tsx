...
type DashboardContextType = {
    ...
    logOut:()=>void
    getAllPage:()=>void
    savePage:(title:string, pageAlias:string, description:ReactQuill.Value, pageImage:File, imageName:string, imageUrl:string, pageType:string, pageId?:number, pageLinkList?:number[], pageImageList?:number[], pageChildList?:{childId:number,childOrder:number}[] )=>Promise<boolean>
...
export const DashboardProvider = ({children}:DashboardProviderType)=>{ 
     ...
     * @author Anil
     */
    const savePage = async (title:string, pageAlias:string, description:ReactQuill.Value, pageImage:File, imageName:string, imageUrl:string, pageType:string, pageId?:number, pageLinkList?:number[], pageImageList?:number[], pageChildList?:{childId:number,childOrder:number}[])=>{
        ...
        // save image file and generate its path
        // if not set the available imageUrl to tempImageUrl
        if(pageImage){
            const imgStorageRef = storageRef(storage, `images/${imageName}`);

            await uploadBytes(imgStorageRef, pageImage)
            .then(async ()=>{
                tempImageUrl = await getImageUrl(imageName);
        ...
        // During edit, if pageType is image then reset its linklists and pageImagelist to empty. 
        ...
        await set(ref(db, `/pages/${id}`), 
            {
                ...
                pageChildList:(pageChildList && pageChildList?.length >0) ? pageChildList: []
            })
        .then(()=>{
    ...
    const deletePage = async (pageId:number, pageImage:string, pageWithSelectedAsChild:DataRowType[])=>{
        // remove page as child from parent pages's pageChildList before deleting the page.
        pageWithSelectedAsChild?.forEach(async (page)=>{
            let pageRef = ref(db,`pages/${page.pageId}`);

            let childList = page.pageChildList.filter((tempPage)=>tempPage.childId !== pageId);

            await update(pageRef, {pageChildList:childList}).then(()=>{   
                console.log('Parent revoked successfully');
            }).catch((e)=>{
                console.log(e);
            });
