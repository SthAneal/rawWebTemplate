...
type FieldPropsType = {
    ...
    multiChildSelectSource?:{pageId:number, pageAlias:string, pageOrder:number, isSelected:boolean, isActive:boolean}[]
    ...
    onOrderChangedFnc?:(childId:number, childOrder:number)=>void
}
...
// text area
export const TextArea = ({ name, label, labelFor, title, required, rows, cols, errorMessage, flex, width, maxWidth, minWidth, defaultValue, value, alignSelf, margin, padding, className, onChangeFnc}:FieldPropsType)=>{
    return(
        <FlexDiv 
            flex={flex?flex:'1 1 auto'} 
            width={width?width:'100%'} 
...
// custom multiple select
export const MultipleChildSelect = React.forwardRef<HTMLElement, FieldPropsType>(({label, required, errorMessage, flex, width, maxWidth, minWidth, alignSelf, margin, padding, onCapsuleClickFnc, multiChildSelectSource, onOrderChangedFnc}:FieldPropsType, ref)=>{
    return(
        <FlexDiv 
            flex={flex?flex:'1 1 auto'} 
            width={width?width:'100%'} 
            ...
            <FlexDiv flex="1" flexWrap="wrap" gap="20px" padding="10px">
                    { (multiChildSelectSource && multiChildSelectSource?.length>0 )? multiChildSelectSource?.map((item)=>{
                        return(
                            <div className={`capsule ${item.isSelected?'capsule__added':''}  ${!item.isActive && 'capsule__hide'}`} key={item.pageId} 
                                onClick={()=> onCapsuleClickFnc && onCapsuleClickFnc(item.pageId)}
                                title={item.isSelected?'Click to unselect': 'Click to select'}
                            >
                                ...
                                <span className="capsule__title">{item.pageAlias}</span>
                                {item.isSelected?<span className="capsule__dropdown" onClick={(e:React.MouseEvent<HTMLElement>)=>e.stopPropagation()}>
                                    <label htmlFor={`childOrder${item.pageId}`}>Order:</label>
                                    <select 
                                        name="childOrder" 
                                        id={`childOrder${item.pageId}`} 
                                        value={item.pageOrder?item.pageOrder:0}
                                        onChange={(e:React.ChangeEvent<HTMLSelectElement>)=> onOrderChangedFnc && onOrderChangedFnc(item.pageId, parseInt(e.target.value))}>
